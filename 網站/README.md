...
> ⚠️ **做任何拖曳元件都會踩到的坑**：拖曳中的半透明狀態
> （`.drag.lift` / `.fpc.lift`，`opacity:.35`）**不能只在 `dragend` 清掉**。
> 放下的時候如果把元素 `appendChild` 到別的容器，元素換了父節點，
> 瀏覽器就不保證會再發 `dragend`——那一塊會一直是半透明的。
> `literacy.js` 的流程拼圖踩過這個，修法是**在「放下」的那一刻就清掉**，
> 再加一道「把所有非拖曳中的碎片掃乾淨」的保險。
